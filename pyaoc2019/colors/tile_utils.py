...
from pyaoc2019.colors.colors import RGB, Color, Colors
...
class ColorMatrix(List[List[Color]]):
    ...
    def resize(self, shape: Shape = (8, 8)) -> 'ColorMatrix':
        ...
        return res.cast(lambda rgb: Color.from_rgb(RGB(*rgb)))
...
def a_star(lower_right_rc: RC, start: RC, end: RC, impassable: Set[RC] = frozenset(), allow_wrap=False):
    """return a* path for maze"""
    start_n = ANode(None, start)
    shape_ul = RC(0, 0)
    shape_lr = lower_right_rc
